...
function getAggregationSql(assetId, aggregation) {
...
export function refreshAggregatedEnergyDataTables(assetId, emileDB) {
    ...
    for (const [tableType, tableName] of Object.entries(tableNames)) {
        ...
        emileDB
          ...
          ${getAggregationSql(assetId, tableType)}`)
            .run();
    }
}
...
export async function ensureEnergyDataTablesExists(assetId, connectedEmileDB) {
    if (energyDataTableNames.size === 0) {
        await reloadEnergyDataTableNames(connectedEmileDB);
    }
    const tableNames = getEnergyDataTableNames(assetId);
    if (energyDataTableNames.has(tableNames.raw) &&
    ...
    const emileDB = connectedEmileDB ?? (await getConnectionWhenAvailable());
    for (const [tableType, tableName] of Object.entries(tableNames)) {
        const createTableSql = tableType === 'raw'
